...
function isOrderByChild(query, fn) {
    return query.identity.orderBy === RealTimeClient.RealQueryOrderType.orderByChild;
}
...
const orderByValue$1 = (list, direction = exports.SortOrder.asc) => {
    const A = direction === exports.SortOrder.asc ? 1 : -1;
...
const sortFn = (query) => query.identity.orderBy === RealTimeClient.RealQueryOrderType.orderByChild
...
class Query {
    constructor(path, delay = 5) {
        this.path = (typeof path === 'string'
            ? path
            : RealTimeClient.SerializedRealTimeQuery.path);
        this._delay = delay;
        this._query =
            typeof path === 'string' ? RealTimeClient.SerializedRealTimeQuery.path(path) : path;
    }
    get ref() {
        return this;
    }
    ...
    equalTo(value, key) {
        if (key && this._query.identity.orderBy === RealTimeClient.RealQueryOrderType.orderByKey) {
...
var freeGlobal = typeof RealTimeClient.commonjsGlobal == 'object' && RealTimeClient.commonjsGlobal && RealTimeClient.commonjsGlobal.Object === Object && RealTimeClient.commonjsGlobal;
...
function getDb(path) {
    return path ? RealTimeClient.lodash_get(db, dotify(path)) : db;
}
/**
 * **setDB**
 *
...
function setDB(path, value, silent = false) {
    ...
    const oldRef = RealTimeClient.lodash_get(db, dotPath);
    ...
    if (value === null) {
        const parentValue = RealTimeClient.lodash_get(db, getParent(dotPath));
        if (typeof parentValue === 'object') {
            delete parentValue[getKey(dotPath)];
            lodash_set(db, getParent(dotPath), parentValue);
        }
        else {
            lodash_set(db, dotPath, undefined);
        }
    }
    else {
        lodash_set(db, dotPath, value);
    }
    if (!silent) {
...
function updateDB(path, value) {
    const dotPath = join(path);
    const oldValue = RealTimeClient.lodash_get(db, dotPath);
    let changed = true;
...
function multiPathUpdateDB(data) {
    ...
    Object.keys(data).map((key) => {
        ...
        const path = key;
        if (RealTimeClient.lodash_get(db, path) !== value) {
            // silent set
    ...
    getListeners().forEach((listener) => {
        ...
        const changeObject = eventPathsUnderListener.reduce((changes, path) => {
            ...
            if (dotify(listener.query.path) === path) {
                ...
                lodash_set(changes, dotify(relativePath(path, listenerPath)), data[path]);
            }
            return changes;
        }, {});
        ...
        const newResponse = {
            ...
            priorValue: listener.eventType === 'value'
                ? RealTimeClient.lodash_get(dbSnapshot, listener.query.path)
                : justValue(RealTimeClient.lodash_get(dbSnapshot, listener.query.path)),
        };
...
class Reference extends Query {
    static createQuery(query, delay = 5) {
        if (typeof query === 'string') {
            query = new RealTimeClient.SerializedRealTimeQuery(query);
        }
...
class SnapShot {
    ...
    child(path) {
        const value = RealTimeClient.lodash_get(this._value, path, null);
...
async function addListener(pathOrQuery, eventType, callback, cancelCallbackOrContext, context) {
    const query = (typeof pathOrQuery === 'string'
        ? new RealTimeClient.SerializedRealTimeQuery(join(pathOrQuery))
        : pathOrQuery);
...
function keyDidNotPreviouslyExist(e, dbSnapshot) {
    return RealTimeClient.lodash_get(dbSnapshot, e.key) === undefined ? true : false;
}
/**
 * **notify**
 *
...
async function importFakerLibrary() {
    if (!faker) {
        faker = await Promise.resolve().then(function () { return require(/* webpackChunkName: "faker-lib" */ './index-e2e9c42d.js'); });
    }
    return faker;
}
/**
...
class Deployment {
    ...
    fulfillBelongsTo(targetSchema) {
        ...
        const relationship = lodash_first(this._relationships
    ...
    insertRelationshipLinks(queue) {
        ...
        belongsTo.forEach((r) => {
            ...
            const recordList = RealTimeClient.lodash_get(db, dotNotation(source.path()), {});
            Object.keys(recordList).forEach((key) => {
                lodash_set(db, `${dotNotation(source.path())}.${key}.${property}`, getID());
            });
        });
        hasMany.forEach((r) => {
            ...
            const property = r.sourceProperty;
            const path = source.path();
            const sourceRecords = RealTimeClient.lodash_get(db, dotNotation(source.path()), {});
            Object.keys(sourceRecords).forEach((key) => {
                for (let i = 1; i <= howMany; i++) {
                    lodash_set(db, `${dotNotation(source.path())}.${key}.${property}.${getID()}`, true);
                }
            });
        });
    }
    ...
    class Queue {
        ...
        dequeue(key) {
            ...
            Queue._queues[this._name] =
                typeof lodash_first(queue) === 'object'
        ...
        toHash() {
            ...
            return typeof lodash_first(queue) === 'object'
                ...
                : queue.reduce((obj, item) => (obj = { ...obj, ...{ [item]: true } }), new Object());
        }
        map(fn) {
        ...
        _find(key) {
            const queue = Queue._queues[this._name];
            const objectPayload = typeof lodash_first(queue) === 'object';
            let index = 0;
            let result = [null, -1];
...
exports.adminAuthSdk = adminAuthSdk;
exports.auth = auth$1;
exports.authMockApi = authMockApi;
