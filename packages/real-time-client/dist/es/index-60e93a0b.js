import { a as RealQueryOrderType, S as SerializedRealTimeQuery, c as commonjsGlobal, l as lodash_get } from './RealTimeClient-876df8ac.js';
import 'events';
...
function getDb(path) {
    return path ? lodash_get(db, dotify(path)) : db;
}
/**
 * **setDB**
 *
...
function setDB(path, value, silent = false) {
    ...
    const oldRef = lodash_get(db, dotPath);
    ...
    if (value === null) {
        const parentValue = lodash_get(db, getParent(dotPath));
...
function updateDB(path, value) {
    const dotPath = join(path);
    const oldValue = lodash_get(db, dotPath);
    let changed = true;
    ...
    getListeners().forEach((listener) => {
        ...
        const newResponse = {
            ...
            priorValue: listener.eventType === 'value'
                ? lodash_get(dbSnapshot, listener.query.path)
                : justValue(lodash_get(dbSnapshot, listener.query.path)),
        };
...
class SnapShot {
    ...
    child(path) {
        const value = lodash_get(this._value, path, null);
...
function keyDidNotPreviouslyExist(e, dbSnapshot) {
    return lodash_get(dbSnapshot, e.key) === undefined ? true : false;
}
/**
 * **notify**
 *
...
async function importFakerLibrary() {
    if (!faker) {
        faker = await import(/* webpackChunkName: "faker-lib" */ './index-eb70feea.js');
    }
    return faker;
}
/**
...
class Deployment {
    ...
    insertRelationshipLinks(queue) {
        ...
        belongsTo.forEach((r) => {
            ...
            const recordList = lodash_get(db, dotNotation(source.path()), {});
        ...
        hasMany.forEach((r) => {
            ...
            const property = r.sourceProperty;
            const path = source.path();
            const sourceRecords = lodash_get(db, dotNotation(source.path()), {});
...
export { Deployment, Mock, MockHelper, Query, Queue, Reference, Schema, SchemaHelper, SnapShot, SortOrder, addListener, adminAuthSdk, auth$1 as auth, authMockApi, clearDatabase, findChildListeners, findValueListeners, getDb, getFakerLibrary, getListeners, getMockHelper, groupEventsByWatcher, importFakerLibrary, listenerCount, listenerPaths, multiPathUpdateDB, notify, pushDB, removeAllListeners, removeDB, removeListener, reset, restoreEvents, setDB, shouldSendEvents, silenceEvents, updateDB, updateDatabase };
