...
describe('Test parameterized mock built-in fakes', () => {
  it('number min/max works', () => {
    for (let i = 0; i < 100; i++) {
      const val = fakeIt({}, 'number', { min: 1, max: 10 });
  ...
  it('number min/max works with negatives', () => {
    for (let i = 0; i < 100; i++) {
      const val = fakeIt({}, 'number', { min: -10, max: 0 });
  ...
  it('number precision 0 works', () => {
    for (let i = 0; i < 100; i++) {
      const val = fakeIt({}, 'number', { min: 1, max: 10, precision: 0 });
  ...
  it('price min/max works, currency symbol defaults to $', () => {
    for (let i = 0; i < 100; i++) {
      const val = fakeIt({}, 'price', { min: 1, max: 100 });
  ...
  it('price symbol can be overwritten', () => {
    for (let i = 0; i < 10; i++) {
      const val = fakeIt({}, 'price', { min: 1, max: 100, symbol: '£' });
  ...
  it('number mocks can set a max and min value which will be respected', async () => {
    for (let i = 0; i < 100; i++) {
      const response = fakeIt({}, 'number', { min: 1, max: 25 });
      expect(response).toBeGreaterThan(0);
      expect(response).toBeLessThan(26);
    }

    for (let i = 0; i < 100; i++) {
      const response = fakeIt({}, 'number', { min: 50, max: 99 });
