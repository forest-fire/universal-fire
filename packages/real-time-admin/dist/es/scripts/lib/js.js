...
export async function transpileJavascript(options = {}) {
    console.log(chalk.bold.yellow(`- starting JS build process ${options.configFile ? "[ " + options.configFile + " ]" : ""}`));
    console.log(chalk.dim(`- transpiling typescript ( `) +
        chalk.dim.grey(`./node_modules/.bin/tsc ${options.scope}`) +
        chalk.dim(` )`));
    try {
        await asyncExec(`./node_modules/.bin/tsc ${options.configFile ? "-p " + options.configFile : ""} ${options.scope}`);
