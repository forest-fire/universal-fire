...
(async () => {
    ...
    const currentVersion = String(await asyncExec(`node -p 'require("./package.json").version'`, {
        silent: true
    })).trim();
    console.log(chalk.dim(`- In your ${chalk.bold.white("package.json")} the current version is`, currentVersion));
    const info = JSON.parse(await asyncExec(`yarn info --json`, { silent: true }));
    ...
    if (currentVersion === npmVersion) {
        console.log(chalk.red.bold(`- Versions are the same, update your package.json before deploying 💩`));
    }
    else {
        try {
